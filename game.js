...
class SoundController {
    ...
    updateUI(playing) {
        ...
        if (playing) {
            ...
            if (label) label.textContent = '«A Place For Always»';
            if (btn) btn.classList.remove('music-pulse');
        } else {
            if (icon) icon.textContent = '🔇';
            if (label) label.textContent = 'Саундтрек';
        }
    }
    ...
    startMusic(callback) {
        if (!this.bgm) return;
        if (!this.bgm.src || !this.bgm.currentSrc) {
...
// ============================================================================
// 8. АСИНХРОННАЯ РАЗБЛОКИРОВКА САУНДТРЕКА (ZERO FREEZE / LAZY STREAM)
// ============================================================================
// При загрузке страницы аудиофайл (6 МБ) НЕ загружается, чтобы не тормозить 3D сцену.
// Кнопка саундтрека подсвечивается мягким золотым сиянием:
const btnMusicEl = document.getElementById('btn-music');
if (btnMusicEl) btnMusicEl.classList.add('music-pulse');

// Слушатели первого взаимодействия: аудио запрашивается и начинает играть
// ТОЛЬКО при первом физическом жесте (клик, тап, свайп) или нажатии кнопки
const unlockEvents = ['pointerdown', 'touchstart', 'touchend', 'mousedown', 'click', 'keydown'];
const tryUnlockAudio = () => {
    sound.init();
    if (!sound.isMusicPlaying) {
        sound.startMusic((success) => {
            if (success) {
                unlockEvents.forEach(evt => {
                    window.removeEventListener(evt, tryUnlockAudio, true);
                    document.removeEventListener(evt, tryUnlockAudio, true);
                });
                showNotification('🎵 Саундтрек: «A Place For Always»');
            }
        });
    }
};
...
unlockEvents.forEach(evt => {
    window.addEventListener(evt, tryUnlockAudio, { capture: true, passive: true });
    document.addEventListener(evt, tryUnlockAudio, { capture: true, passive: true });
});

// Отложенная попытка тихого автозапуска через 1.5 сек после полной инициализации 3D сцены (если браузер позволяет)
setTimeout(() => {
    if (!sound.isMusicPlaying) {
        sound.startMusic((started) => {
            if (!started) {
                showNotification('🎵 Нажмите в любом месте для включения звука');
            }
        });
    }
}, 1500);
